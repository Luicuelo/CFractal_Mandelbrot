...
void renderFractal(void)
{
	...
	sprintf(s, "Mouse (%d,%d) Iterations: %d", mouse_move_x,mouse_move_y, max_iterations);
...
void rescaleView(int pmouse_down_x,int pmouse_down_y,int pmouse_up_x,int pmouse_up_y)
{
	...
	if (complex_step_x < 1e-6) {
		sprintf(s, "Complex Plane: (%.8f,%.8f) : (%.8f,%.8f)", complex_origin_x, complex_origin_y, complex_origin_x + complex_step_x*WINDOW_WIDTH, complex_origin_y + complex_step_y*WINDOW_HEIGHT);
	} else {
...
void handleMouseMove(int x, int y, HWND hwnd)
{
	...
	if (isImageLoaded) {
		...
		sprintf(s, "Mouse (%d,%d) Iterations: %d - RENDERING...", mouse_move_x,mouse_move_y, max_iterations);
	}
	updateStatusBar(s, 0, 0);
	...
	if (isImageLoaded && isButtonPressed)
	{
		...
		if (complex_step_x < 1e-6) {
			sprintf(s, "Complex Plane: (%.8f,%.8f) : (%.8f,%.8f)", complexStartX, complexStartY, complexEndX, complexEndY);
		} else {
